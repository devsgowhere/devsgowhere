...
class EventCLI {
  ...
  private async scrapeEventDataFromMeetup(page: Page): Promise<ScrapedEventData> {
	...
	// get event description from "main #event-details" as html
	...
	if (eventDescription) {
		...
		scrapedData.description = eventDescription;
	} else {
		console.warn(`No description found for the event.`);
		scrapedData.description = '';
	}

	// @todo: convert description from html to markdown
	// scrapedData.description = convertHtmlToMarkdown(scrapedData.description);
  ...
  private async createEventFile(eventData: EventData): Promise<void> {
	...
	try {
	  ...
	  const markdownContent = this.generateMarkdownContent(eventData);

	  // Write the file
	  await fs.writeFile(eventFile, markdownContent, 'utf8');
  ...
  private generateMarkdownContent(eventData: EventData): string {
